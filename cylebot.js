...
function parse_message(message_obj, username, message_type) {
	...
	*/
	...
	if (/^!(.+)$/.test(chatline)) {
		...
		//Primary functionality.  "Tell me about ?" should bring a Warframe Wiki link to that item.
		if (/^Tell me about (\w+).{0,1}/i.test(chatline)) {
			var wiki_pieces = chatline.match(/Tell me about (\w+).{0,1}$/i);
			if (wiki_pieces == undefined || wiki_pieces[1] == undefined) {
				say("I can't answer a non-question", where);
				return;
			}
			var item = wiki_pieces[1];
			item = item.replace(/ /g, "_");
			item = item.replace(/./, '');
			say("Look here for what you need to know", where);
			say("warframe.wikia.com/wiki/" + item);
		}
